 ...
 ******************************************************************************************************/
class Magentodesarrollo_Fintonic_Block_Payment_Qrcode extends Mage_Payment_Block_Info
{
	...
	protected function _toHtml()
    {
		...
		if ($_POST['phone']!=""){
			...
			$phone = $order_address_billing['telephone'];
		}

		$phoneResponse = Mage::getSingleton('checkout/session')->getphoneresponse();


		$cont = Mage::getSingleton('checkout/session')->setcont(0);
		
		if($phoneResponse != null && $phoneResponse == 'ok'){
			$alert = 'success';
		}
		
		if($phoneResponse = null && $cont > 0){
			$alert = 'fail';
		}
...
<html lang="en">
  <head>
    ...
    function alertFooter(alertType){
	...
	function modifyPhone() {
        document.getElementById("modifyPhone").submit();
	}
    </script>
  </head>
  <body onload="heightModal()">
    ...
    <div class="window-fixed">
        <div id="modal" class="modal">
            ...
            <h3>Para continuar, necesitamos que sigas en tu teléfono móvil</h3>
            <div class="content">
                <div id="alertSuccess" class="alert-success">
                  <p>
                    SMS enviado. Abre el link del mensaje y podrás continuar con la solicitud de pago a plazos desde tu teléfono móvil. Fintonic te ayudará durante el proceso.
                  </p>
                </div>
                <div id="alertDanger" class="alert-danger">
                  <p>
                    Lo sentimos, ha ocurrido un problema al procesar tu solicitud. Por favor, inténtalo más tarde.
                  </p>
                </div>
                <div class="column first">
                    <p>
                        Te enviaremos un <b>SMS</b> a tu teléfono móvil con un link para que puedas continuar con la solicitud de pago a plazo
                    </p>
                    <div class="box-sms">
                    <form id="modifyPhone" action="'.Mage::getUrl("fintonic/payment/modify", array("_secure" => true)).'" method="post">
                        <label>Teléfono móvil:</label> <input type="number" value="'.$phone.'" name="phone" placeholder="'.$phone.'" />
                    </form>
                    </div>
                    <div class="align-center">
                        <button class="btn-submit" type="submit" onclick="modifyPhone()">Recibir SMS</button>
                    </div>
                </div>
                <div class="column">
                    <p>
                      Tambiém puedes escanear este <b>código QR</b> con tu teléfono para continuar con la solicitud.
                    </p>
                    <div class="align-center">
                        <img class="qr-image" width="150" height="150" src="'.Mage::getSingleton("checkout/session")->geturlimage().'" />
                    </div>
                </div>
            </div>
        </div>
        <form id="canceltFin" action="'.Mage::getUrl("fintonic/payment/success", array("_secure" => true)).'" method="post" >

        </form>
    </div>
    <script> alertFooter("'.$alert.'");</script>
  </body>
  </html>
